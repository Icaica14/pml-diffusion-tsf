...
Evaluation *is* the project's rigor. We measure three families + enforce statistical hygiene.
...
### 6.4 Statistical hygiene (do not skip — it's cheap originality)
...
### 6.5 Evaluation pitfalls to actively guard against
...
Always ask it to add a tiny test or a `__main__` smoke check. Review every diff against Part 6.5 pitfalls.
