...
10. **Conclusion** — "more expressiveness is not automatically better calibration: in our setting the seasonal-naive is unbeaten on CRPS and DeepAR is the best-calibrated trained model; diffusion's value is conditional and comes at a measurable, tunable sampling cost — which we can price." + future work (Bayesian/ensemble diffusion; calibration).
